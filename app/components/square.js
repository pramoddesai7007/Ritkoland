    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const temporaryOrdersResponse = await axios.get('https://backend1-ashen.vercel.app/api/order/temporary-orders-count');
                ...
                const totalForCurrentDateResponse = await axios.get('https://backend1-ashen.vercel.app/api/order/total-amount-for-current-date');
                setCounts(prevCounts => ({
                    ...prevCounts,
                ...
                const totalForPreviousMonthResponse = await axios.get('https://backend1-ashen.vercel.app/api/order/total-amount-for-previous-month');
                setCounts(prevCounts => ({
                    ...prevCounts,
                    totalForPreviousMonth: totalForPreviousMonthResponse.data.totalForPreviousMonth,
                }));

                const totalBalanceResponse = await axios.get('https://backend1-ashen.vercel.app/api/purchase/purchases-total');
                setCounts(prevCounts => ({
                    ...prevCounts,
